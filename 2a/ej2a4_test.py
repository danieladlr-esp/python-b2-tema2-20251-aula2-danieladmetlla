...
def test_ax1_features():
    ...
    assert ax1.title.get_text() == "Monthly Sales Comparison: 2020 vs 2021", "The title of ax1 should be correct"
    assert len(ax1.patches) == 24, "There should be 24 bars in the bar chart (12 for each year)"
...
def test_ax2_features():
    ...
    assert ax2.title.get_text() == "2022 Monthly Sales Distribution", "The title of ax2 should be correct"
    assert len(ax2.patches) == 12, "There should be 12 segments in the pie chart (one for each month)"
...
def test_data_label_length_match():
    ...
    assert len(sales_2020) == len(months), "Length of sales data and month labels must match for the first year"
    assert len(sales_2021) == len(months), "Length of sales data and month labels must match for the second year"
    assert len(sales_2022) == len(months), "Length of sales data and month labels must match for the third year"
