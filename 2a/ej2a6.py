...
def gaussian_fit_and_integration(data_x: t.List[float], data_y: t.List[float]) -> t.Tuple[t.Tuple[float], float]:
...
def plot_gaussian_fit(data_x: t.List[float], data_y: t.List[float], gaussian_params: t.Tuple[float]):
    plt.figure(figsize=(10, 6))
    plt.scatter(data_x, data_y, label='Datos Originales', color='blue')
    plt.plot(data_x, gaussian(data_x, *gaussian_params), label='Ajuste Gaussiano', color='red')
    plt.xlabel('Eje X')
    plt.ylabel('Eje Y')
