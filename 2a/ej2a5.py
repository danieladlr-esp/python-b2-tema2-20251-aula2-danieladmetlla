...
def linear_regression_and_interpolation(data_x: t.List[float], data_y: t.List[float]) -> t.Dict[str, t.Any]:
    # Regresión Lineal
...
def plot_results(data_x: t.List[float], data_y: t.List[float], results: t.Dict):
    ...
    # Regresión lineal
    plt.plot(data_x, results['linear_regression']['slope'] * data_x + results['linear_regression']['intercept'],
             label='Regresión Lineal', color='red')

    # Interpolación
    plt.plot(data_x, results['interpolated_data'], label='Interpolación', color='green', linestyle='--')
