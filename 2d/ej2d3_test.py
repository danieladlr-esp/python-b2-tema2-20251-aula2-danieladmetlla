...
def test_perform_linear_regression(housing_data):
    slope, intercept, r_value, p_value, std_err = perform_linear_regression(housing_data, 'RM', 'MEDV')
...
def test_plot_regression_line(housing_data):
    ...
    intercept = -1
    fig, ax = plot_regression_line(housing_data, 'RM', 'MEDV', slope, intercept, return_fig_ax_test=True)
    assert ax.get_title() == "Linear Regression between RM and MEDV", "The chart title is not correct"
