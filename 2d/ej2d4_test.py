...
from ej2d4 import prepare_data, train_svm_classifier, perform_svm_classification, predict_species, target_names

@pytest.fixture
def iris_dataset_path():
    return './data/iris_dataset.csv'
...
def test_prepare_data(iris_dataset_path):
    ...
    assert len(X_train) / (len(X_train) + len(X_test)) == pytest.approx(0.8, 0.05), \
...
def test_predict_species(trained_classifier):
    ...
    assert prediction in target_names.values(), "The prediction should be one of the target names"
