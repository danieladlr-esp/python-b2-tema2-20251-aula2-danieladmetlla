...
def test_read_csv_multi_index():
    ...
    assert isinstance(df, pd.DataFrame), "The returned object is not a pandas DataFrame"
    assert not df.empty, "The DataFrame is empty"
    assert isinstance(df.index, pd.MultiIndex), "The DataFrame index is not a MultiIndex"
...
def test_read_csv_custom_separator():
    ...
    assert isinstance(df, pd.DataFrame), "The returned object is not a pandas DataFrame"
    assert not df.empty, "The DataFrame is empty"
    assert df['Stars'].dtype == float or df['Stars'].dtype == 'float64', "'Stars' column is not of type float"
