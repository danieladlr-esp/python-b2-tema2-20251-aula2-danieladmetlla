...
def test_read_population_data():
    ...
    assert all(isinstance(table, pd.DataFrame) for table in tables), "Each item should be a DataFrame"
...
def test_get_table_by_string_match():
    ...
    assert match_text in matched_table.to_string(), f"The table should contain the text {match_text}"
