...
def read_population_data(url: str, match_text: str = None) -> t.List[pd.DataFrame]:
    ...
    attrs = {'class': 'wikitable'}  # Asumiendo que queremos tablas con la clase 'wikitable'
    if match_text:
...
def get_table_by_string_match(tables: t.List[pd.DataFrame], match_text: str) -> t.Union[pd.DataFrame, None]:
    try:
        for table in tables:
