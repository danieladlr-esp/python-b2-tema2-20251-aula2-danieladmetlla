...
escenarios comunes en el análisis de datos mediante Pandas y la función 'read_json()'. Se proporciona un archivo JSON y se
...
Las funciones y escenarios a desarrollar son:
    ...
    DataFrame de Pandas.
    - Leer JSON con orientación específica: read_json_with_orientation(file_path, orient) que lee un archivo JSON con una
    orientación específica (por ejemplo, 'split', 'records', 'index').
...
Cada función debe devolver un DataFrame de Pandas, permitiendo a los estudiantes ver el efecto de diferentes modos de lectura de archivos JSON.
...
Ejemplo:
    ...
    df_normalized = read_json_and_normalize(json_normalize_path, record_path=['data', 'ratings'])

    # Mostrar los primeros registros de cada DataFrame
    print(df_basic.head(), df_orient.head(), df_normalized.head())
