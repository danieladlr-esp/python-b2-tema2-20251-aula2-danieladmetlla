...
@pytest.fixture
def excel_file_path():
    return 'data/ej2b4/ramen-ratings.xlsx'
...
def test_read_excel_custom_sheet(excel_file_path):
    ...
    assert not df.columns.str.contains(
        'Unnamed').any(), "Columns should not contain 'Unnamed' which implies empty columns."
    # Verify that the expected columns and rows are present
    expected_columns = ['Brand', 'Country', 'Review #', 'Stars', 'Style', 'Top Ten', 'Variety']
    ...
    assert len(df) == expected_row_count, (f"The DataFrame should have the expected number of rows, after skipping the"
                                           f"footer.")
